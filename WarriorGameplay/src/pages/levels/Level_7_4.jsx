...
const Level_7_4 = () => {
  ...
  useEffect(() => {
    ...
    if (healthbar) {
      ...
      observer.observe(healthbar, { attributes: true, attributeFilter: ['class'] });
    ...
    if (waiting && malakarothHp > 0 && !isGameOver) {
      interval = setInterval(() => {
        ...
        setMalakarothHp((prev) => {
          ...
          if (newHp === 0) {
            setIsGameOver(true);
            addToBattleLog("Victory! Malakaroth has been defeated by the passage of time!");
          }
          return newHp;
        });
  ...
  const addToBattleLog = (message) => {
    setBattleLog(prev => [...prev, { message, timestamp: new Date().toLocaleTimeString() }]);
  };
  ...
  const handleMalakarothAttack = () => {
    if (!isPlayerTurn && !isGameOver) {
      ...
      addToBattleLog(`Malakaroth unleashes a devastating attack for ${damage} damage!`);
  ...
  const handleWait = () => {
    if (isPlayerTurn && !isGameOver) {
      setWaiting(true);
      setIsPlayerTurn(false);
      addToBattleLog("You choose to wait, watching as time affects Malakaroth...");
    }
  };
          ...
          <div className="text-2xl font-bold text-center mb-6">
            {!isGameOver && (
              <div className={`inline-block px-6 py-2 rounded-full ${
                isPlayerTurn ? "bg-blue-900 text-blue-300" : "bg-red-900 text-red-300"
