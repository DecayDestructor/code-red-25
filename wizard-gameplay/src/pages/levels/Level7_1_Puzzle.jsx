...
const Level7_1 = () => {
  ...
  const script =
    'U2VhbGluZyBzcGVsbDpTb21ldGhpbmcgbG9zdCBpcyBzb21ldGhpbmcgZ2FpbmVkLiAKClNlZSB0aGUgZGV2ZWxvcGVyIHRvb2xzLg==' //Something lost is something gained
  window.spell = script
  ...
  const fileInfo = [
    {
      ...
      content: Array.from(
        { length: 7 },
        () => `Try command ${generateRandomCommand()}<br>`
      ).join('\n'),
    },
    {
      name: 'notes.log',
  ...
  const commands = [
    ...
    },
    {
      name: 'ls',
      textIfPasswordNotAddedAndNotSudo: `<br><span style="margin-left:1rem;">Permission denied. </span> <br>`,
      textIfPasswordAddedAndSudo: `<br>${fileInfo.map(
        (file) => file.name + '<br>'
      )}`,
    },
    {
  ...
  return (
    <>
      <div className="w-screen h-screen bg-stone-950 overflow-x-hidden">
        <div
          ref={dummyDivRef}
          id="something-lost-is-something-gained"
          className="hidden"
        ></div>
        <div
          ref={contentRef}
